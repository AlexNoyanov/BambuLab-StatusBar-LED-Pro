...
#ifndef PROGRESS_SHOW_RED_REMAINING
  #define PROGRESS_SHOW_RED_REMAINING true  // false = green line only, no red
#endif
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (doc["print"]["gcode_state"]) {
    ...
    if (strcmp(state, "RUNNING") == 0) printerState = Printing;
      ...
      if (printerState != Finished) {
        soundPlayed = false; // Reset sound flag to play melody
      }
  ...
  if (printerState == Error) {
    ...
    if (doc["print"]["mc_percent"] || (doc["print"]["gcode_state"] && strcmp(doc["print"]["gcode_state"], "RUNNING") == 0)) {
      ...
      finishedSince = millis();
      setAll(CRGB::Green);
      soundPlayed = false;  // Play finish melody
    }
  }
    ...
    case Printing:
      {
        float pct = progressPercent / 100.0;
        int onLeds = (int)(NUM_LEDS * pct + 0.5);
        CRGB offColor = PROGRESS_SHOW_RED_REMAINING ? PROGRESS_OFF_LED : CRGB::Black;
        for (int i = 0; i < NUM_LEDS; i++) {
          int ledIndex = (progressDirection == 1) ? i : (NUM_LEDS - 1 - i);
          leds[ledIndex] = (i < onLeds) ? PROGRESS_ON_LED : offColor;
        }
        break;
      }
...
void setup() {
  ...
  // Play startup melody right away (before WiFi) so it's audible
   initSound();
  //playStartupMelody(); (Working)
...
void loop() {
  ...
  mqttClient.loop();
  updateLEDs();
  updateSound(printerState, finishedSince);
  delay(67);
}
